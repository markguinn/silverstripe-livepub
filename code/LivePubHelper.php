...
class LivePubHelper extends Object
{
     ...
     */
    public static function init_pub()
    {
        global $project;
     ...
     */
    public static function get_init_code()
    {
        if (self::is_publishing()) {
            ...
            if (count(self::$vars) > 0) {
                ...
                foreach (self::$vars as $var => $valCode) {
                    if ($valCode === true) {
                        continue;
                    } // this means it was already set in the initialization
                    $code .= '$' . $var . ' = ' . $valCode . ";\n";
                }
                $code .= "\n?>\n";
            }
     ...
     */
    public static function add_init_code($code, $id = null)
    {
        if ($id) {
     ...
     * @return string
     */
    public static function get_init_code_and_clear()
    {
     ...
     * @return string
     */
    public static function eval_php($code)
    {
        if (self::is_publishing()) {
     ...
     * @return none
     */
    public static function exec_php($code, $alwaysExec=false)
    {
        if (self::is_publishing()) {
            self::$init_code[] = $code;
            if ($alwaysExec) {
     ...
     * @return string
     */
    public static function include_php($filename)
    {
     ...
     * @param array $paths
     */
    public static function set_template_paths(array $paths)
    {
     ...
     * @param string $path
     */
    public static function add_template_path($path)
    {
        if (!isset(self::$template_path) || empty(self::$template_path)) {
            self::init_template_paths();
        }
        if ($path[0] != '/') {
            $path = BASE_PATH . '/' . $path;
     ...
     */
    public static function wrap($object, $class = 'ViewableWrapper', $add_init_code=true)
    {
        if (self::is_publishing()) {
            ...
            if ($add_init_code && ($init = $obj->getStaticInit())) {
                self::$init_code[] = $init;
            }
        } else {
            $obj = new $class($object);
     ...
     */
    public static function wrap_set(array $list, $class = 'ViewableWrapper', $exclude=null, $exclude_field=null)
    {
        $set = new DataObjectSet();

        foreach ($list as $r) {
            $obj = self::wrap($r, $class);
            if ($exclude && in_array($obj->getField($exclude_field), $exclude)) {
                continue;
            }
            $set->push($obj);
        }
